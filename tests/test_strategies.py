...
def _build_history(prices):
    ...
    history.trading_history_df = pd.DataFrame(
        {
            "AAA": [0] * len(stock_df),
            "bank_account": [100.0] * len(stock_df),
            "money_invested": [0] * len(stock_df),
        },
...
def test_strategy_no_investment_is_noop():
    prices = pd.DataFrame(
        {"Close": [10.0]}, index=pd.date_range("2020-01-01", periods=1)
    )
...
def test_strategy_marcus_accrues_interest():
    ...
    assert history.trading_history_df.at[prices.index[0], "bank_account"] == approx(
        100.0 + expected_growth
    )


def test_strategy_maf_triggers_buy_on_crossover(monkeypatch):
    prices = pd.DataFrame(
        {"Close": [1.0, 2.0, 3.0, 2.5]}, index=pd.date_range("2020-01-01", periods=4)
    )
    history = _build_history(prices)
    buy_calls = []
    monkeypatch.setattr(
        history, "buy_all_shares", lambda *args, **kwargs: buy_calls.append(args)
    )
    monkeypatch.setattr(
    ...
    monkeypatch.setattr(
        "python_stocks.dougs_strategies.slope", lambda *_args, **_kwargs: 1.0
    )
...
def test_strategy_openclose_uses_prior_close_vs_open(monkeypatch):
    prices = pd.DataFrame(
        {"Close": [1.0, 0.5]}, index=pd.date_range("2020-01-01", periods=2)
    )
    ...
    history.stock_df_to_today = stock_df
    history.trading_history_df = pd.DataFrame(
        {"AAA": [5, 5], "bank_account": [0.0, 0.0], "money_invested": [0, 0]},
        index=prices.index,
    )
    ...
    buy_calls = []
    monkeypatch.setattr(
        history, "sell_all_shares", lambda *args, **kwargs: sell_calls.append(args)
    )
    monkeypatch.setattr(
        history, "buy_all_shares", lambda *args, **kwargs: buy_calls.append(args)
    )
