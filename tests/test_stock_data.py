...
def _make_stub_df():
    ...
    return pd.DataFrame({"Date": dates, "Close": [1.0, 2.0, 3.0, 4.0], "Open": [1.0, 2.0, 3.0, 4.0]})


def test_get_stock_df_to_today_and_money_to_add(monkeypatch):
    stub_df = _make_stub_df().set_index("Date")
    monkeypatch.setattr(stock_data_module, "load_into_stock_data_set", lambda _: stub_df)
...
def test_limit_timeframe_trims_bounds(monkeypatch):
    ...
    assert list(stock_data.data_frame.index) == list(pd.date_range("2020-01-02", periods=2, freq="D"))
