...
def _risk_return(snapshot: pd.Series, window: int, cost_penalty: float) -> Tuple[float, float, float]:
...
def comparison_matrix_figure(
    sample: MarketSample, ticker: str, theme: Theme, window: int = 60, cost_penalty: float = 0.002
) -> go.Figure:
    ...
    for symbol in sample.tickers:
        ...
        points.append({"symbol": symbol, "return": mean_return * 100, "vol": vol * 100, "cost": est_cost * 100})
    ...
    fig.add_trace(
        go.Scatter(
            ...
            marker={
                ...
                "color": [theme["accent"] if p["symbol"] == ticker else theme["accent_alt"] for p in points],
...
def timeline_overlay_figure(sample: MarketSample, ticker: str, theme: Theme, horizon: int = 120) -> go.Figure:
    ...
    if not close_series.empty:
        start, mid, end = close_series.index[0], close_series.index[len(close_series) // 2], close_series.index[-1]
        fig.add_vrect(x0=start, x1=mid, fillcolor=theme["panel"], opacity=0.08, line_width=0)
        fig.add_annotation(
            x=start,
            y=cumulative.max(),
